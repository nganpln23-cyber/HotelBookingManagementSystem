...
public class PromotionRepository {
    ...
    public PromotionRepository(JdbcTemplate jdbcTemplate) {
    ...
    public List<Promotion> findActive() {
        return jdbcTemplate.query(
            "SELECT * FROM promotions WHERE active=1 ORDER BY min_completed_bookings", mapper);
    }

    public Promotion findByCode(String code) {
        try {
            return jdbcTemplate.queryForObject(
                "SELECT * FROM promotions WHERE code=? AND active=1", mapper, code);
